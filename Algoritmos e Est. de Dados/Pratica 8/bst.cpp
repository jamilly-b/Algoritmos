...
void BinaryTree::show(Node *root) {
  if (root == NULL) return;
  // TODO
  // Exibe a árvore em ordem (in-order): esquerda, raiz, direita
  // Deve exibir o valor da chave e a altura por nó da árvore
  // Formato: (<valor>, <altura>), separado por espaços, exemplo:
  // (1, 0) (3, 2) (2, 0) ... (10, 3)
  show(root->left);
  cout <<" (" << root->key << ", " << H(root) << ") ";
  show(root->right);


}

//******* A arvore visualmente *******
//      10
// 1         35
//   7     22  45
//       16      88
...
BinaryTree::Node *BinaryTree::remove(BinaryTree::Node *root, int key) {
  ...
  if (root == NULL)
    return NULL; // (sub)árvore vazia, nada a fazer
...
int BinaryTree::validate(Node *root, int &min, int &max) {
  ...
  // Se árvore à esquerda é inválida ou maior � esquerda > raiz então INVÁLIDA
  if (!validate(root->left, lmin, lmax) || (lmax > root->key))
    return 0;

  // Se árvore à direita é inválida ou menor � direita < raiz então INVÁLIDA
