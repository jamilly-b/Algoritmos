...
void searchRemoveShow(BinaryTree &tree, int value, int show) {
  ...
  cout << "Value: " << value << " Found/removed = " << (found ? "YES" : "NO ")
       << " [" << (valid ? "valid" : "invalid") << "]";
...
int main() {
  ...
  try {
    ...
    for (int i = 1; i < size - 1; i++) {
      ...
      cout << "Successor   of " << val << " == " << succ << "? "
           << ((tree->successor(val) == succ) ? "YES" : "NO") << endl;
      cout << "Predecessor of " << val << " == " << pred << "? "
           << ((tree->predecessor(val) == pred) ? "YES" : "NO") << endl;
    }
